...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    This function accepts a dataframe, 2 columns(feature) and 2 values which returns the dataframe based on the condition.
    Keyword arguments:
...
# Code to calculate different aggreagated values according to month
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    df[date_col] = df[date_col].apply(lambda x: pd.to_datetime(str(x)))
    return(df.pivot_table(index=df[date_col].dt.month,values=agg_col,aggfunc=agg))
...
agg_values_ina_month(weather,'Date/Time','Temp (C)','sum')
...
mean_weather = group_values(weather,"Weather",{'Temp (C)':'mean','Dew Point Temp (C)':'mean','Rel Hum (%)':'mean','Wind Spd (km/h)':'mean','Visibility (km)':'mean','Stn Press (kPa)':'mean'})
